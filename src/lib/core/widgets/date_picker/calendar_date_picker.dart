...
const double _maxDayPickerHeightM2 =
    _dayPickerRowHeightM2 * (_maxDayPickerRowCount + 1);
const double _maxDayPickerHeightM3 =
    _dayPickerRowHeightM3 * (_maxDayPickerRowCount + 1);
...
class _CalendarDatePickerState extends State<CalendarDatePicker> {
  ...
  void didChangeDependencies() {
    ...
    if (!_announcedInitialDate && widget.initialDate != null) {
      ...
      final bool isToday =
          widget.calendarDelegate.isSameDay(widget.currentDate, _selectedDate);
    ...
    setState(() {
      ...
      if (_selectedDate case final DateTime selected) {
        final String message = switch (mode) {
          DatePickerMode.day =>
            widget.calendarDelegate.formatMonthYear(selected, _localizations),
          DatePickerMode.year =>
            widget.calendarDelegate.formatYear(selected.year, _localizations),
        };
    ...
    setState(() {
      if (_currentDisplayedMonthDate.year != date.year ||
        ...
        _currentDisplayedMonthDate =
            widget.calendarDelegate.getMonth(date.year, date.month);
  ...
  void _handleYearChanged(DateTime value) {
    ...
    final int daysInMonth =
        widget.calendarDelegate.getDaysInMonth(value.year, value.month);
    final int preferredDay = math.min(_selectedDate?.day ?? 1, daysInMonth);
    value =
        widget.calendarDelegate.getDay(value.year, value.month, preferredDay);
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      switch (Theme.of(context).platform) {
        ...
        case TargetPlatform.macOS:
        case TargetPlatform.windows:
          final bool isToday = widget.calendarDelegate
              .isSameDay(widget.currentDate, _selectedDate);
          final String semanticLabelSuffix =
              isToday ? ', ${_localizations.currentDateLabel}' : '';
          SemanticsService.announce(
  ...
  Widget build(BuildContext context) {
    ...
    final double scaledMaxDayPickerHeight = textScaleFactor > 1.3
        ? maxDayPickerHeight +
            ((_maxDayPickerRowCount + 1) * ((textScaleFactor - 1) * 8))
        : maxDayPickerHeight;
    return Stack(
      children: <Widget>[
        SizedBox(
...
class _DatePickerModeToggleButtonState
  ...
  Widget build(BuildContext context) {
    ...
    //         defaults.subHeaderForegroundColor;
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void initState() {
    ...
    _pageController = PageController(
      initialPage:
          widget.calendarDelegate.monthDelta(widget.firstDate, _currentMonth),
    );
  ...
  void _handleMonthPageChanged(int monthPage) {
    setState(() {
      ...
      if (!widget.calendarDelegate.isSameMonth(_currentMonth, monthDate)) {
        _currentMonth =
            widget.calendarDelegate.getMonth(monthDate.year, monthDate.month);
        ...
        SemanticsService.announce(
          widget.calendarDelegate
              .formatMonthYear(_currentMonth, _localizations),
          _textDirection,
        );
      }
    });
  ...
  DateTime? _focusableDayForMonth(DateTime month, int preferredDay) {
    final int daysInMonth =
        widget.calendarDelegate.getDaysInMonth(month.year, month.month);
    ...
    for (int day = 1; day <= daysInMonth; day++) {
      final DateTime newFocus =
          widget.calendarDelegate.getDay(month.year, month.month, day);
      if (_isSelectable(newFocus)) {
        return newFocus;
      }
    }
  ...
  void _showMonth(DateTime month, {bool jump = false}) {
    final int monthPage =
        widget.calendarDelegate.monthDelta(widget.firstDate, month);
    if (jump) {
  ...
  bool get _isDisplayingFirstMonth {
    return !_currentMonth.isAfter(
      widget.calendarDelegate
          .getMonth(widget.firstDate.year, widget.firstDate.month),
    );
  }
  ...
  bool get _isDisplayingLastMonth {
    return !_currentMonth.isBefore(
      widget.calendarDelegate
          .getMonth(widget.lastDate.year, widget.lastDate.month),
    );
  }
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (widget.calendarDelegate
            .isSameMonth(widget.selectedDate, _currentMonth)) {
          _focusedDay = widget.selectedDate;
        } else if (widget.calendarDelegate
            .isSameMonth(widget.currentDate, _currentMonth)) {
          _focusedDay =
                ...
                ...List.generate(
                  ...
                  (index) {
                    return InkWell(
                      ...
                      onTap:
                          index == 0 ? _handlePreviousMonth : _handleNextMonth,
                      child: Padding(
          ...
          child: FocusableActionDetector(
            ...
            child: _FocusedDate(
              ...
              child: PageView.builder(
                ...
                itemCount: widget.calendarDelegate
                        .monthDelta(widget.firstDate, widget.lastDate) +
                    1,
...
class _FocusedDate extends InheritedWidget {
  const _FocusedDate(
      {required super.child, required this.calendarDelegate, this.date});
...
class _DayPickerState extends State<_DayPicker> {
  ...
  void didChangeDependencies() {
    ...
    if (focusedDate != null &&
        widget.calendarDelegate
            .isSameMonth(widget.displayedMonth, focusedDate)) {
  ...
  Widget build(BuildContext context) {
    ...
    final int dayOffset =
        widget.calendarDelegate.firstDayOffset(year, month, localizations);
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final DateTime dayToBuild =
            widget.calendarDelegate.getDay(year, month, day);
        ...
        final bool isToday =
            widget.calendarDelegate.isSameDay(widget.currentDate, dayToBuild);
    ...
    return Padding(
      ...
      child: MediaQuery.withClampedTextScaling(
        ...
        child: GridView.custom(
          ...
          childrenDelegate:
              SliverChildListDelegate(dayItems, addRepaintBoundaries: false),
        ),
      ),
    );
  }
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    final RoundedRectangleBorder dayShape = RoundedRectangleBorder(
          ...
          style: AppTextStyles.small.copyWith(
            ...
            color: () {
              ...
              if (widget.isSelectedDay) {
                return AppColors.projectBackground;
              }
    ...
    Widget yearItem = Center(
      child: Container(
        ...
        child: Semantics(
          selected: isSelected,
          button: true,
          child: Text(widget.calendarDelegate.formatYear(year, localizations),
              style: itemStyle),
        ),
      ),
    );
...
class _YearPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    final double tileWidth = (constraints.crossAxisExtent -
            (scaledYearPickerColumnCount - 1) * _yearPickerRowSpacing) /
